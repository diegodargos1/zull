...
class FieldEdit extends React.Component{
    ...
    onChangeFunction(e){
        if(e.target.name == 'cnpj' && e.target.value.length > 14){
            this.setState({
                fieldMask: "99.999.999/0009-99",
            })
        }else if(e.target.name == 'cnpj' && e.target.value.length <= 14){
            this.setState({
                fieldMask: this.props.mask,
            })
        }
        this.setState({
